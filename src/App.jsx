...
function App() {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("user"); // YENİ: Çıkış yaparken kullanıcı bilgisini de temizle
    setToken(null);
  };
            ...
            {filteredMedia.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {filteredMedia.map((media) => (
                  <div
                    key={media._id}
                    className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow group relative"
                  >
                    <div className="absolute top-2 right-2 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => handleEdit(media)}
                        className="bg-blue-500/80 hover:bg-blue-600 text-white p-1.5 rounded-full"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button
                        onClick={() => handleDelete(media._id)}
                        className="bg-red-500/80 hover:bg-red-600 text-white p-1.5 rounded-full"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between items-center mt-3 text-xs text-slate-500">
                        <span
                          className={`px-2 py-1 rounded-full ${media.status === "Completed" ? "bg-green-100 text-green-700" : media.status === "In Progress" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700"}`}
                        >
                          {media.status === "To Do"
                            ? "Yapılacak"
