...
import { Trash2, Edit2, Search, Filter, LogOut } from "lucide-react"; // LogOut ikonu eklendi
...
function App() {
  ...
  const fetchMedia = async () => {
    try {
      const response = await fetch("https://media-tracker-api.onrender.com/api/media", {
        headers: {
          // Eğer Backend tarafında JWT koruması açtıysak bu satır zorunludur:
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`https://media-tracker-api.onrender.com/api/media/${id}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` } // Güvenlik için token gönderiyoruz
      });
  ...
  // 3. EFFECT'LER
  useEffect(() => {
    // Sadece token varsa verileri çek
    if (token) {
      fetchMedia();
    }
  }, [token]);
  ...
  // 5. GİRİŞ YAPILMAMIŞSA (TOKEN YOKSA) AUTH EKRANINI GÖSTER
  if (!token) {
    ...
    <div className="min-h-screen bg-slate-50 p-8 font-sans text-slate-900">
      <header className="max-w-7xl mx-auto mb-8">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
          ...
          {/* Çıkış Yap ve Yeni Ekle Butonları Yan Yana */}
          <div className="flex gap-3">
            <button
              onClick={handleAddNew}
              className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl font-medium shadow-lg shadow-blue-600/20 transition-all active:scale-95"
            >
              + Yeni Ekle
            </button>
            <button
              onClick={handleLogout}
              className="flex items-center gap-2 bg-red-100 hover:bg-red-200 text-red-600 px-4 py-2.5 rounded-xl font-medium transition-all active:scale-95"
            >
              <LogOut size={18} />
              Çıkış
            </button>
      ...
      <AddMediaModal
        ...
        token={token} // Güvenlik için token'ı modal'a da gönderdik
      />

      <div className="max-w-7xl mx-auto">
        {loading ? (
          <div className="text-center text-slate-500 mt-20">Yükleniyor...</div>
        ) : (
          <>
                ...
                {filteredMedia.map((media) => (
                  <div key={media._id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow group relative">
                    <div className="absolute top-2 right-2 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleEdit(media)} className="bg-blue-500/80 hover:bg-blue-600 text-white p-1.5 rounded-full">
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => handleDelete(media._id)} className="bg-red-500/80 hover:bg-red-600 text-white p-1.5 rounded-full">
                        <Trash2 size={16} />
                      </button>
                    </div>
                    ...
                    <div className="h-72 bg-slate-200 relative">
                      <img
                        src={media.imageUrl || "https://via.placeholder.com/300x450?text=No+Image"}
                        alt={media.title}
                    ...
                    <div className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex items-center gap-1 bg-yellow-50 px-2 py-0.5 rounded text-yellow-700 font-bold text-sm">
                          <span>★</span>{media.rating}
                        </div>
                      </div>

                      <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden mt-3">
                        <div
                          className={`h-full transition-all duration-500 ${media.status === "Completed" ? "bg-green-500" : "bg-blue-500"}`}
